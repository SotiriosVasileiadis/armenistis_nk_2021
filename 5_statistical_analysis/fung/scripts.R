...
my_tax_tbl_2 <- my_tax_tbl[which(my_tax_tbl$Domain == "Fungi"),]
## Fix the annotation
...
combomatfin <- combomat[grep("Fungi", combomat$Domain),]
...
myrarecurve <- rarecurve(t(otumatfin)[order(row.names(t(otumatfin))),],step=50, sample = raremax, col = factor(design[order(row.names(t(otumatfin))),]$plant), cex = 0.6, xlim= c(0,15000))
...
# plot the first line
plot(x=as.numeric(gsub("N","",names(myrarecurve[[1]]))),y=myrarecurve[[1]], type = "lines", col = desgnfrrar[1], xlim=c(0,15000), ylim=c(0,800), frame.plot = F, ylab = "OTUs", xlab = "Sequence #")
...
plot(mysamp, type = "n", bty="n", main = mymain, cex.main = 0.8)
...
legend(x = -2.5, y = -1.2, cex=0.7, col = c("black"), lty = c(1,2), lwd = 2, bty = "n", legend = levels(design2$Aromatic), y.intersp = 1, text.font = 1)
legend(x = -2.5, y = -1.5, legend = levels(design2$Aromatic), cex=0.7, col=1:2, pch = 19, bty= "n", bg = "transparent", text.font = 1, y.intersp = 1)
legend(x = -2.5, y = -1.8, legend = levels(design2$season), cex=0.7, col="black", pch = c(21,24), bty = "n", text.font = 1, y.intersp = 1)
...
# prepare the colour/ellipse keys/legends
legend(x = -2.1, y = -1.1, cex=0.7, col = c("black"), lty = c(1,2), lwd = 2, bty = "n", legend = levels(design2$type_canopy), y.intersp = 1, text.font = 1)
legend(x = -2.1, y = -1.45, legend = levels(design2$type_canopy), cex=0.7, col=1:2, pch = 19, bty= "n", bg = "transparent", text.font = 1, y.intersp = 1)
legend(x = -2.1, y = -1.8, legend = levels(design2$season), cex=0.7, col="black", pch = c(21,24), bty = "n", text.font = 1, y.intersp = 1)
...
for(my_tax_lev in my_tax_lvls[3:length(my_tax_lvls)]){
  ...
  # aggregate the replicates (one replicate is missing from M. communis summer samples due to low seq counts)
  ...
  mycols <- c(rainbow10equal,"grey70","grey80")
  # run the plotting function
  p <- plot_bar2(my_phydata_rafr_plot_bar, fill = my_tax_lev, toptaxa = 10)
  ...
  p <- plot_bar2(my_phydata_rafr_plot_bar, fill = my_tax_lev, toptaxa = 10) + facet_wrap(~season, scales="free_x") + theme(strip.background =element_rect(fill ="white"))
...
for(myreqrelabun in c(0.02,0.04,0.05)){
